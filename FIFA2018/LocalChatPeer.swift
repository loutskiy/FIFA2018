...
class LocalChatPeer: NSObject {
    ...
    */
    func start(_ displayName: String) {
        ...
        let blueetoothModule = BluetoothModule(type: "SimpleP2PChat", dispatchQueue: DispatchQueue.main)
        let remoteModule = RemoteP2PModule(baseUrl: NSURL(string: "ws://localhost:8080/")! as URL, dispatchQueue: DispatchQueue.main)
        localPeer = LocalPeer(name: displayName, modules: [wlanModule, blueetoothModule, remoteModule], dispatchQueue: DispatchQueue.main)
